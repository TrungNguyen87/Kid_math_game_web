...
### Added (round 11 - Direct connection: WebRTC with manual signaling, zero backend)

**Round 9 and round 10 both landed on the same wall: a real join-code race
needs something authoritative keeping everyone's clock and question in
sync, and a static site has nowhere to run that - so online play worked
self-hosted (`npm start`) and simply didn't exist on the published GitHub
Pages site.** SESSIONS.md session 9 restated this as the thing a future
session shouldn't try to "fix" by adding a server to Pages, because Pages
doesn't run one. This round closes the gap a different way: instead of a
server relaying messages between two browsers, the two browsers open a
direct `RTCDataChannel` to each other with WebRTC, and the one thing WebRTC
still needs a third party for - the initial offer/answer handshake, usually
carried by a signaling server - is carried by the two players themselves
instead. Host and guest are, on the same wifi or LAN, typically within
arm's reach of each other anyway.

- **The handshake is manual, on purpose.** The host creates a room and gets
  an "offer" - a block of text (and, where it's short enough, a QR code of
  the same text) - shows it to the guest, who reads it (scan or paste),
  produces an "answer" the same way, and hands it back. Once the host
  applies the answer, ICE has everything it needs and the data channel
  opens on its own. No account, no relay, nothing but the two devices
  involved ever sees any of it - the same "no third party" posture as
  round 9's challenge links, just now for a live connection instead of a
  finished result.
- **No STUN or TURN server, deliberately.** `iceServers: []` means only
  "host" candidates (the device's own LAN address) are gathered - exactly
  what two devices on the same subnet need, per the brief this round
  implements, and it keeps the offer/answer blob small enough to fit
  comfortably in a QR code. Reaching for a public STUN server would have
  added a dependency on an external service this app otherwise has none of,
  for a scenario (same LAN) that doesn't need one.
- **The room logic didn't need to change, only where it runs.**
  `race-server.js`'s `RaceRoomManager` never actually depended on
  WebSockets except in one method (`attachWebSocketServer`) - everywhere
  else it only ever called `ws.send()` and checked `ws.readyState`. Pulled
  the class out into **`web/js/race-room-engine.js`** (framework-agnostic,
  imports only `race-logic.js`), and `race-server.js` now just adds the
  `ws` wiring back around it. The Direct connection mode runs this exact
  same class *in the host's own browser tab*: the host's own player is a
  loopback "socket" (`send()` calls the page's event handler directly, no
  network), the guest is the real data channel wrapped the same minimal
  way. This is why nothing about the lobby, the countdown, a round, the
  recap or the results screen needed to change - `web/js/pages/compete.js`
  already spoke to a `{send(msg), stop()}` client and rendered off the
  `type`-tagged messages that client received; a WebRTC client satisfying
  that same shape was a drop-in third option next to the existing
  WebSocket/REST one.
- **`web/js/webrtc-signal.js`** - the offer/answer handshake itself:
  creating the host's offer and the guest's answer, waiting for ICE
  gathering to finish before either is shown (manual signaling can't do
  trickle ICE - there's no channel to trickle candidates over until the
  channel exists), and `encodeSignalBlob`/`decodeSignalBlob`, which
  base64url-encode `{version, offer|answer, room code, settings, sdp}` and,
  on decode, treat every field as hostile input and range-check all of it -
  the same posture round 9's `decodeChallenge` took, because a pasted blob
  can be hand-edited or truncated exactly like a pasted link could.
- **`web/js/webrtc-race-client.js`** - `WebRtcHostClient` and
  `WebRtcGuestClient`, the two ends of the connection, each matching the
  existing `RaceClient`'s `{send(msg), stop()}` shape (see above).
- **`web/js/qrcode.js`** - a QR code encoder (byte mode), adapted from
  Kazuhiko Arase's widely-used MIT-licensed `qrcode-generator`: same tables
  (Reed-Solomon block sizes, alignment-pattern positions, the BCH
  generator polynomials for the format/version info - all copied, not
  retyped from memory, because a subtly wrong table still draws *a* grid of
  squares, just one no camera can read), trimmed to only what this app
  uses (no numeric/alphanumeric/kanji modes, no canvas/table/GIF renderers)
  and rebuilt as a native ES module that draws its output as one SVG
  `<path>` in this app's own inline-SVG style rather than the original's
  string-built HTML tags.
- **`web/js/pages/compete.js`** - a new "connection method" choice inside
  Online: **📶 Direct** (this round, the default - it's the one that works
  everywhere this app is deployed) and **🖧 Via a server** (round 10's
  WebSocket room, renamed in the UI to make the self-hosted requirement
  explicit rather than implied). Direct's create/join flow is new; the
  lobby, countdown, round, recap and results screens are the *same
  functions* round 10 already had, now also fed by a `WebRtcHostClient`/
  `WebRtcGuestClient` instead of only a `RaceClient`.
- 26 new `compete.*` i18n keys, both languages, for the new flow (send this
  code / paste their reply / connecting / try again, etc.), plus optional
  QR scanning: a "📷 Scan QR code" button appears next to the paste box
  wherever the browser's own `BarcodeDetector` API is available (Chrome/
  Edge/Android today), opening an inline camera preview and treating a
  detected code exactly like a paste - the moment a code is found is the
  submission, no second confirming tap, the same "a tap is the answer"
  rule the rest of this app already follows. Nowhere else, the button
  simply doesn't render and the text box is, as always, the one thing that
  always works.

**Why not just add a signaling server, hosted for free somewhere?** That
would reintroduce exactly the third party and the "needs its own account"
problem this app has avoided since round 6 - and it wouldn't even be
necessary for the stated scenario (both devices on the same subnet), where
WebRTC's own ICE candidate gathering already does the "find each other on
the LAN" part for free, once the two ends know each other's SDP.

- Tests: `tests/web/test_logic.mjs` gained round-trip tests for
  `qrcode.js` that are worth calling out specifically - since the tables
  are copied from elsewhere, what is actually this file's own risk is the
  *wiring* between them, so the test decodes the matrix the encoder
  produces independently of the encoder's own state (reads the format-info
  bits, unmasks, walks the data region in the same zig-zag order written
  fresh for the test rather than imported, and checks the error-correction
  codewords with a from-scratch Reed-Solomon syndrome calculation - a
  different computation, polynomial evaluation instead of polynomial
  division, so a wrong generator polynomial or a wrong interleave order
  shows up as a non-zero syndrome rather than agreeing with itself) across
  several versions, including one that exercises alignment patterns and
  version-info bits (a first pass at this test had exactly that bug: it
  reserved alignment-pattern zones that overlap a finder pattern too
  eagerly, which the round-trip decode caught immediately as a syndrome
  mismatch - structural checks like "is there a finder pattern" would not
  have). Also new: `race-room-engine.js`'s room lifecycle (create, join,
  fill a room, start, answer, finish) driven directly with fake
  `{send, readyState}` sockets, with no transport at all - and
  `webrtc-signal.js`'s blob encode/decode round-trip and its adversarial
  cases (wrong blob type, truncated, garbage, tampered SDP). `tests/web/
  smoke.mjs`'s existing online test now explicitly selects "Via a server"
  (Direct is the default, and that server-backed path only exists
  self-hosted); a new test drives the whole Direct handshake between two
  real browser pages - reading the offer straight out of the DOM instead of
  scanning a QR code, but it is exactly the text a QR code carries - through
  a real `RTCDataChannel`, a started race, and a round recap. Run twice by
  hand during this round: once against `node server.js` and once against a
  bare `python3 -m http.server` serving only `web/` with **no Node process
  behind it at all** - the same shape as the published GitHub Pages site -
  to actually prove the "zero backend" claim rather than assume it from
  reading the code.
- `web/sw.js`'s precache list gained the four new files;
  `tools/check_precache.py` passes (52 files).
